...
def perguntar_oraculo(texto):
    # Lista de modelos por ordem de prioridade
    modelos = ['gemini-1.5-flash', 'gemini-1.5-pro', 'gemini-pro']
    for m in modelos:
        try:
            model = genai.GenerativeModel(m)
            res = model.generate_content(f"Responda como um sábio: {texto}")
            return res.text
        except Exception:
            continue
    return "O Oráculo está em silêncio profundo agora. Tente novamente em instantes."
...
with t1:
    if ak:
        p = st.chat_input("Diga algo ao Oráculo...")
        if p:
            with st.spinner("O Oráculo está refletindo..."):
                resposta = perguntar_oraculo(p)
                st.chat_message("assistant").write(resposta)
    else: st.error("Falta a chave gemini_api nos Secrets.")
    ...
    if au:
        try:
            ...
            if q:
                ...
                res = s.files().list(q=f, fields=flds).execute().get('files', [])
