...
# --- SCANNER DE MODELOS DISPONÍVEIS ---
@st.cache_resource
def buscar_modelo_valido(api_key):
    try:
        genai.configure(api_key=api_key)
        # Lista todos os modelos que a sua chave tem permissão para usar
        for m in genai.list_models():
            if 'generateContent' in m.supported_generation_methods:
                return m.name # Retorna o primeiro que funcionar (ex: models/gemini-pro)
    except: return None
    return None

nome_modelo = buscar_modelo_valido(ak) if ak else None
...
with t1:
    if not nome_modelo:
        st.error("Nenhum modelo de IA disponível para esta chave. Tente gerar uma nova chave no AI Studio.")
    else:
        p = st.chat_input("Diga algo...")
        if p:
            with st.spinner(f"Refletindo com {nome_modelo}..."):
                try:
                    md = genai.GenerativeModel(nome_modelo)
                    r = md.generate_content(p)
...
with t2:
    if au:
        try:
            ...
            q = st.text_input("Nome do arquivo:")
            if q:
                res = s.files().list(q=f"name contains '{q}'", fields="files(name,webViewLink)").execute().get('files', [])
