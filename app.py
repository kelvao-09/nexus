...
# 1. Configuração da IA (Pegando a chave dos Secrets)
...
pergunta = st.text_input("O que deseja saber sobre seus documentos?")

if pergunta:
    if "gemini_api" not in st.secrets:
        st.error("Por favor, adicione a 'gemini_api' nos Secrets do Streamlit.")
    else:
        with st.spinner("Consultando o Oráculo..."):
            # O prompt envia a pergunta para a IA
            res = model.generate_content(f"Você é o sábio Oráculo. Responda: {pergunta}")
            st.chat_message("assistant").write(res.text)

# 5. Busca de Arquivos Original
st.divider()
@st.cache_resource
def get_s():
...
s = get_s()
q = st.text_input("🔍 Buscar Arquivos no Drive", placeholder="Digite o nome...")
